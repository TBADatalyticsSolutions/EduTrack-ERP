...
from apps.academics.models import AcademicSession, Term, SchoolClass
...
class FeeCategory(BaseModel):
    ...
    class Meta:
        ...
        constraints = [models.UniqueConstraint(fields=["school", "name"], name="finance_fee_category_school_name_uniq")]
...
class FeeStructure(BaseModel):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=["school", "session", "term", "school_class", "fee_category"], name="finance_fee_structure_scope_uniq"),
            models.CheckConstraint(condition=models.Q(amount__gte=0), name="finance_fee_structure_amount_gte_0"),
        ]
...
class StudentInvoice(BaseModel):
    ...
    class Meta:
        ordering = ["-created_at"]
        constraints = [
            models.UniqueConstraint(fields=["school", "student", "session", "term"], name="finance_invoice_student_session_term_uniq"),
            models.CheckConstraint(condition=models.Q(total_amount__gte=0), name="finance_invoice_total_gte_0"),
            models.CheckConstraint(condition=models.Q(balance__gte=0), name="finance_invoice_balance_gte_0"),
        ]
...
class InvoiceItem(BaseModel):
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(condition=models.Q(amount__gte=0), name="finance_invoice_item_amount_gte_0"),
            models.CheckConstraint(condition=models.Q(paid_amount__gte=0), name="finance_invoice_item_paid_gte_0"),
        ]

    def clean(self):
        errors = {}
        if self.invoice_id and self.fee_category_id and self.invoice.school_id != self.fee_category.school_id:
...
class Payment(BaseModel):
    SETTLEMENT_TYPES = (("PAYMENT", "Payment"), ("SCHOLARSHIP", "Scholarship / Full Waiver"), ("WAIVER", "Fee Waiver / Adjustment"))
    PAYMENT_METHODS = (("CASH", "Cash"), ("TRANSFER", "Bank Transfer"), ("POS", "POS"), ("ONLINE", "Online"))
    invoice = models.ForeignKey(StudentInvoice, on_delete=models.CASCADE, related_name="payments")
    amount = models.DecimalField(max_digits=12, decimal_places=2)
    settlement_type = models.CharField(max_length=20, choices=SETTLEMENT_TYPES, default="PAYMENT")
    ...
    class Meta:
        constraints = [models.CheckConstraint(condition=models.Q(amount__gte=0), name="finance_payment_amount_gte_0")]
    ...
    def _recalculate_invoice(invoice):
        total_settled = invoice.payments.aggregate(models.Sum("amount"))["amount__sum"] or Decimal("0.00")
        invoice.balance = max(invoice.total_amount - total_settled, Decimal("0.00"))
