            ...
            if current_class.pk == next_class.pk:
                ...
                if "preview" in request.POST:
                    if preview_count == 0:
                        messages.warning(
                            request,
                            f"There are no eligible students in {current_class.name} for promotion.",
                        )
                    else:
                        messages.info(
                            request,
                            f"{preview_count} eligible student(s) found in {current_class.name}.",
                        )
                elif "promote" in request.POST:
                    if preview_count == 0:
                        messages.warning(
                            request,
                            f"There are no eligible students in {current_class.name} to promote.",
                        )
                    else:
                        try:
                            with transaction.atomic():
                                promoted = promote_students(current_class, next_class)
                            log_activity(
                                request,
                                action="PROMOTION",
                                module="Students",
                                description=(
                                    f"Bulk promotion completed. {promoted} student(s) promoted "
                                    f"from '{current_class.name}' to '{next_class.name}'."
                                ),
                            )
                            messages.success(
                                request,
                                f"{promoted} student(s) promoted successfully from {current_class.name} to {next_class.name}.",
                            )
                            return redirect("promotion")
                        except Exception as exc:
                            messages.error(
                                request,
                                f"The promotion could not be completed. Error: {exc}",
                            )
        ...
        if form.is_valid():
            ...
            if success:
                log_activity(
                    ...
                    module="Students",
                    description=(
                        f"Student '{student.full_name()}' was transferred. "
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            log_activity(
                ...
                module="Students",
                description=(
                    f"Student '{student.full_name()}' was graduated. Reason: {reason}"
                ),
            )
    ...
    if request.method == "POST":
        ...
        if class_id:
            ...
            if "graduate" in request.POST:
                if total_students == 0:
                    messages.warning(request, "There are no students to graduate.")
                ...
                log_activity(
                    ...
                    module="Students",
                    description=(
                        f"Bulk graduation completed for class '{selected_class.name}'. "
                        f"{graduated} student(s) graduated."
                    ),
                )
                messages.success(request, f"{graduated} student(s) graduated successfully.")
                return redirect("bulk-graduation")
...
def promote_student_view(request, pk):
    ...
    if request.method == "POST":
        ...
        next_class = get_object_or_404(SchoolClass, pk=next_class_id)
        if student.current_class_id == next_class.id:
            messages.error(request, "The student is already in this class.")
            return redirect("student-promote", pk=student.pk)
        if student.is_graduated:
            messages.error(request, "A graduated student cannot be promoted.")
            return redirect("student-promote", pk=student.pk)
        old_class = student.current_class
        try:
            promoted_student = promote_single_student(student=student, next_class=next_class)
        ...
        log_activity(
            ...
            description=(
                ...
                f"'{old_class.name if old_class else 'Unassigned'}' to '{next_class.name}'."
            ),
        )
        messages.success(
            request,
    ...
    next_classes = SchoolClass.objects.exclude(pk=student.current_class_id).order_by("name")
    return render(
        request,
